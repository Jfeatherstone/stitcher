...
from .topology import findPeaks2D
...
def kernelFind(image, kernelImage, fftPadding=50, paddingValue=None, debug=False):
    """
    ...
    if kernelImage.ndim > 2:
        kernelArr = np.mean(kernelImage, axis=-1)
    else:
        kernelArr = kernelImage
    ...
    halfSizeDiff = [(imageArr.shape[i] - kernelArr.shape[i])//2 for i in range(2)]
    paddedKernelArr = np.zeros((int(imageArr.shape[0] + 2*fftPadding), int(imageArr.shape[1] + 2*fftPadding)))
    paddedKernelArr[halfSizeDiff[0] + fftPadding:-halfSizeDiff[0] - fftPadding,halfSizeDiff[1] + fftPadding:halfSizeDiff[1] - fftPadding] = kernelArr

    # Both dimensions will always have the same first crop index, but possibly a different second one
    cropBounds = [fftPadding, -fftPadding]
    ...
    if paddingValue is None or paddingValue == 0:
        ...
        # Top
        paddedKernelArr[:fftPadding] += np.multiply.outer(2/(1 + np.exp(np.linspace(0, 5, fftPadding))), paddedKernelArr[fftPadding])[::-1,:] 
        # Bottom
        paddedKernelArr[-fftPadding:] += np.multiply.outer(2/(1 + np.exp(np.linspace(0, 5, fftPadding))), paddedKernelArr[-fftPadding-1])
        # Left
        paddedKernelArr[:,:fftPadding] += np.multiply.outer(paddedKernelArr[:,fftPadding], 2/(1 + np.exp(np.linspace(0, 5, fftPadding))))[:,::-1]
        # Right
        paddedKernelArr[:,-fftPadding:] += np.multiply.outer(paddedKernelArr[:,-fftPadding-1], 2/(1 + np.exp(np.linspace(0, 5, fftPadding))))
    else:
        ...
        paddedKernelArr[:fftPadding] = paddingValue
        paddedKernelArr[-fftPadding:] = paddingValue
        paddedKernelArr[fftPadding:-fftPadding,:fftPadding] = paddingValue
        paddedKernelArr[fftPadding:-fftPadding,-fftPadding:] = paddingValue

    # First convolutional term
    convTerm1 = ifft2(fft2(paddedImageArr**2) * fft2(paddedKernelArr))
    # Trim padding
    convTerm1 = convTerm1[cropBounds[0]:cropBounds[1],cropBounds[0]:cropBounds[1]]

    convTerm2 = 2 * ifft2(fft2(paddedImageArr) * fft2(np.ones_like(paddedKernelArr)))

    # Trim padding
    convTerm2 = convTerm2[cropBounds[0]:cropBounds[1],cropBounds[0]:cropBounds[1]]
    ...
    normalizationTerm = ifft2(fft2(originalImageMask) * np.ones_like(paddedKernelArr))

    # Trim padding
    normalizationTerm = np.real(normalizationTerm[cropBounds[0]:cropBounds[1],cropBounds[0]:cropBounds[1]])
